%% BASIC C shape, edges uneven
function [Open] = basicOpenBot(Vars,numkey,numreps)
w = Vars.w;
...
%% RUN EXPERIMENT
  ...
  for itrial = 1:Vars.maxnum*Open.numreps
    ...
    while numsquare > 0
        ...
        % draw bgcolor square over bottom to create open side
    ...
    % clear screen and present mask 
    Screen('FillRect',w,Vars.bgColor, Vars.rect);
    Screen('DrawTexture',w,mask);
    Screen('Flip',w,[],1)
        
    [StoreKey, Secs] = numKeyReport(numkey); % wait for number keypress before proceeding
    ...
    Open.NumResp(itrial)=StoreKey; % response
